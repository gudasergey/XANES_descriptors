...
def readSpectrum(fileName, skiprows=0, energyColumn=None, intensityColumn=None, separator=r'\s+', decimal=".", guess=True):
    fileName = fixPath(fileName)
    ...
    def fixColumnInd(ncols, energyColumn, intensityColumn):
        ...
        if intensityColumn is None:
            if ext == '.nor': intensityColumn = 3
                ...
                assert ncols == 2, f"intensityColumn not set in readSpectrum and ncols > 2"
                intensityColumn = 1
        if ncols == 2:
