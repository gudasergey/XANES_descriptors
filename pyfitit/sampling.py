...
def loadParams(folder, spectralProgram):
    _, res = getParams(os.path.join(folder, 'geometryParams.txt'))
    return res
...
def parse_all_folders(parentFolder, spectral_program, printOutput=True):
    ...
    def separate_folders(allXanes):
        ...
        """
        badFolders = []
        if spectral_program in ['fdmnes', 'fdmnesTest']:
        ...
        for d in allXanes:
            ...
            if spectral_program in ['fdmnes', 'fdmnesTest'] and allXanes[d].intensity.shape[0] != maxEnergyCount:
    ...
    def create_dataframes(allXanes, goodFolders):
        ...
        if spectral_program in ['fdmnes', 'fdmnesTest'] and fdmnes.useEpsiiShift:
            ...
            energies = allXanes[goodFolders[0]].energy
        else:
            assert False
        ...
        for i in range(n):
            ...
            elif spectral_program in ['pyGDM']:
...
def convertToSample(dataset, spectralProgram):
    ...
    df_xanes, df_params, goodFolders, badFolders = parse_all_folders(parentFolder=os.path.split(additionalData[good[0]]['folder'])[0], spectral_program=spectralProgram, printOutput=False)
...
def checkSampleIsGoodByCVError(maxError, smoothConfig, spectralProgram, estimator=None, minCountToCheckError=10, cvCount=10, debug=False, debugOutputFolder='debug', testSample=None):
    """
