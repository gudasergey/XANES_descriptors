...
def plotSample(energy, spectra, color_param=None, sortByColors=False, fileName=None, title='', alpha=1):
    ...
    if color_param is not None:
        ...
        c_min, c_max = np.min(color_param), np.max(color_param)
        transform = lambda r: (r - c_min) / (c_max - c_min)
        colors = transform(color_param)
        colors = plt.cm.gist_rainbow(colors)
        colors = colors[ind]
        ax.set_prop_cycle(cycler.cycler('color', colors))
        if ML.isClassification(color_param):
            ticks = np.unique(color_param)
        else:
            ticks = np.linspace(np.min(color_param), np.max(color_param), 10)
        ticksPos = transform(ticks)
        addColorBar(plt.cm.ScalarMappable(norm=matplotlib.colors.Normalize(vmin=0, vmax=1), cmap=plt.cm.gist_rainbow), fig, ax, {}, '', ticksPos, ticks)
    else:
...
def addColorBar(mappable, fig, ax, labelMaps, label, ticksPos, ticks, format='%.2g'):
